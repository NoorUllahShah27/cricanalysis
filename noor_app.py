...
st.title("Noor Ullah Shah - Cric Info App")

df=pd.read_csv("cricinfo_clean_data.csv")

#st.dataframe(df)

select= option_menu(
    menu_title=None,
    options=["Home","Player Analysis","Country Insights","Comparison","Data Exployer","About"],
...
if select=="Home":
    ...
    piel=df_pie.T.reset_index()
    fig_pie=px.pie(piel,names="index",values=piel.columns[1],color="index")
    ...
    st.title("Country Wise Performance Analysis")
...
elif select=="Data Exployer":
    st.title("Data Exploration")
    st.dataframe(df)

elif select=="About":
    st.info("About this Python Project")
    
    st.text("Project by: Noor Ullah Shah")
    
    st.success("End to end streamlit Data Analysis Dashboard using Python for Cricket Analysis")
    
    url1 = "https://www.linkedin.com/in/noor-ullah-shah-7609b361/"
    
    url2 = "https://github.com/NoorUllahShah27"
